...
However, 3rd-party services are not always free, and most importantly, they can't operate on your local `anvil`. However, the technology for enabling this has been around for a while, and you use it all the time if you develop smart contracts with `forge`. It's just that it's not typically exposed in a way that frontend developers can leverage it.
