export const blogManifest = [
  await import("#/blog/01-announcing-ethui/index.mdx"),
  await import("#/blog/02-ethui-0.2.0-ui-level-up/index.mdx"),
  await import("#/blog/03-ethui-0.3.1-mainnet/index.mdx"),
  await import("#/blog/04-instant-foundry-abi-explorer/index.mdx"),
  await import("#/blog/05-ethui-0.5.0-impersonation/index.mdx"),
  await import("#/blog/06-ethui-0.6.0-going-multi-chain/index.mdx"),
  await import("#/blog/07-ethui-0.7.0-eth-lisbon/index.mdx"),
  await import("#/blog/08-ethui-1.1.1-ledger-devtools/index.mdx"),
  await import("#/blog/09-ethui-1.6-a-new-beginning/index.mdx"),
  await import("#/blog/10-ethui-1.7-forms/index.mdx"),
  await import("#/blog/11-ethui-1.13-were-back/index.mdx"),
  await import("#/blog/12-ethui-1.14/index.mdx"),
];
